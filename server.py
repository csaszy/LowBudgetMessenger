...
class Client:
    ...
    def Send(self, msg):
        ...
        self.conn.sendto(msg.encode(FORMAT),(self.addr[0],self.addr[1]))
    def Recv(self):
        print(f'{self.id} receiving')
        msg_length = self.conn.recv(HEADER).decode(FORMAT) #blocking code --> block the flow until it receves a message
        if msg_length:
